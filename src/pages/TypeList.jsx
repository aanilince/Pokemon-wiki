...
const TypeList = () => {
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      <h1 className="text-3xl font-bold font-display text-vintage-900 border-b border-vintage-400/50 pb-4 inline-block">
        Elemental Types
      </h1>
        ...
        {types.map((type) => (
          <Link
            ...
            className={`${getTypeColor(
              type.name
            )} p-6 rounded-sm shadow-md transform hover:translate-y-[-2px] hover:shadow-lg transition-all flex flex-col items-center justify-center border-double border-4 opacity-90 hover:opacity-100 group`}
          >
            ...
            <span className="text-white font-display uppercase font-bold tracking-widest text-xs text-shadow-sm">
              {type.name}
            </span>
          </Link>
        ))}
